    ...
    if (st) {
        ...
        if (betPlaced) {
            ...
            if (state.condition2Immediate) {
                const afterLossSkip = Number(state.condition2Immediate.afterLossSkip) || 3;
                if (wasWin) {
                    st.level = 1;
...
async function runPredict(userId, chatId) {
    ...
    try {
        if (signal && signal.action && signal.action.immediateBet && !state.isSkipping && !state.skipWatch) {
            ...
            await send(chatId, `⚡ Condition 2 triggered — placing 1 immediate bet (opposite).`);
        }
    } catch (e) {
    ...
    bot.on("message",async msg=>{
        ...
        if(hasAccess(id) && userAction[id]){
            ...
            if(text === "🔙 Back") { delete userAction[id]; }
                ...
                const a = getBetSequence({ ...autobetCfg[id], baseBet: v }, autobetCfg[id].maxLvl);
